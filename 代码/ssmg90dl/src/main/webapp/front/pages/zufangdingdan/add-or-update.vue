<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(51, 51, 51, 1)","textAlign":"left"}' class="title">租房日期</view>
				<picker mode="date" :value="ruleForm.zufangriqi" @change="zufangriqiChange">
					<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"8rpx","color":"rgba(51, 51, 51, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"26rpx","borderStyle":"solid","height":"60rpx"}' class="uni-input">{{ruleForm.zufangriqi?ruleForm.zufangriqi:"请选择租房日期"}}</view>
				</picker>
			</view>
												<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zufangdingdan`, this.ruleForm.id);
		...
		methods: {
			...
			// 多级联动参数
																																													...
																																													zufangriqiChange(e) {
				this.ruleForm.zufangriqi = e.target.value;
				this.$forceUpdate();
			},
			...
			async onSubmitTap() {
					...
					return
																																																																																																																																																																																																																																																																																																...
																																																																																																																																																																																																																																																																																																if(this.ruleForm.id){
					await this.$api.update(`zufangdingdan`, this.ruleForm);
				}else{
					await this.$api.add(`zufangdingdan`, this.ruleForm);
				}
